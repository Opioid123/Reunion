    ...
    public class FileUploadController : Controller
    {
        ...
        public async Task<HttpResponseMessage> Upload(int projectId, int sectionId)
        {
            ...
            try
            {
                ...
                {
                    var filesReadToProvider = await Request.Content.RadAsMultipartAsync();
                    var index = 0;
        ...
        private void SaveToFolder(Image img, Size newSize, string pathToSave)
        {
            ...
            using (System.Drawing.Image newImg = new Bitmap(img, imgSize.Width, imgSize.Height))
            {
                ...
                if (System.IO.File.Exists(_env.ContentRootPath + pathToSave))
                    System.IO.File.Delete(_env.ContentRootPath + pathToSave);

                newImg.Save(_env.ContentRootPath + pathToSave, img.RawFormat);                
            }
        }
